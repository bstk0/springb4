...
@Component
public class PrestadorDAO { //implements DAOInterface<Cultura> {

    private static final String COLLECTION = "prestador";
    private static final String ROOT_COLLECTION = "rep1_prestador";
...
//        try {
            ...
            Sysout.s( " >> PrestadorDAO.getList - results : " + results.size());
            this.NCOUNT = results.size();

        //List<HPeople> hPeopleList = this.getListFromJSON(results); //resultGetAll);
        //resultGetAll);
        return UtilsJSON.getListFromJSON(results,Prestador.class);
    }

    public void add(Prestador hpeople) {
        JSONObject snuttgly = hpeople.toJSON();
    ...
    public void update(Prestador hPeople) {
    ...
    public Prestador getItem(int id) {
        return this.getItem(String.valueOf(id));
    }


    public Prestador getItem(String id) {
        ...
        try {
            ...
            imov = objectMapper.readValue(obj.toString(), Prestador.class);
